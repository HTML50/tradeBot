    ...
    function randomDecimal(level) {
        ...
        if (debug) console.warn('完全随机小数')
            ...
            var random = Math.round(Math.random() * rangeLevel),
                ...
                console.log(minLevel,random,level)
...
function startAutoTrade(){
    if (!running) {
        ...
        //异步。
        //现获取交易价格，然后，从storage取出参数，开始。
/*         var getAmount = getInitValuePromise('amountTotal','amount')
         var getHigh =  getInitValuePromise('priceHigh','high')
         var getLow=  getInitValuePromise('priceLow','low')
         var getMin=  getInitValuePromise('amountMin','min')
         var getMax=  getInitValuePromise('amountMax','max')
         var getRandomLevel=  getInitValuePromise('isRandomLevel','max')*/


    //Promise.all([getAmount,getHigh,getLow,getMax,getMin]).then(function() {
        if(debug) {
            vital('任务参数：')
        ...
        var cancel = false
        var cancelProcess = setTimeout(function() {
            error('网络错误，无法初始化。刷新重试，若仍旧不行，请检查VPN或网络状态。')
        }, 5000)
          ...
          xhr.open('GET', 'https://api.coinegg.im/api/v1/ticker?coin=oc');
            xhr.onreadystatechange = function(){
                if(xhr.readyState === 4 && xhr.status ===200){
                    ...
                    initBuy = eval('data.'+ajaxBuy);
                    initSell = eval('data.'+ajaxSell);

                    clearTimeout(cancelProcess)
...
function getInitValuePromise(variableName,id){
    var item = KEY  + id
    return new Promise(function(resolve, reject) {
          chrome.storage.sync.get([item],function(result){
                if(typeof result[item] !== 'undefined') {
                   window[variableName] = Number(result[item]);
                   resolve();
                }
            })
        });
        ...
        if (xhr.readyState == 4) {
            ...
            if (xhr.status == 200) {
                ...
                if (data.last && safeRequest) {
                    ...
                    if(running) autoTradeProcess = setTimeout(trade, 1000);
                }
            } else {
